...
| `exempt_ips`         | `tuple[str, ...]`       | `()`              | IPs/CIDRs that skip rate limiting and the user-agent and cloud-provider checks. Not restrictive: other IPs are checked as usual. The blacklist, dynamic bans, route IP rules and penetration detection still apply. |
